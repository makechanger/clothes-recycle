...
> 9. **密码 BCrypt 加密（已完成）**：所有密码使用 Hutool BCrypt 加密存储和校验。
> 10. **权限控制（已完成）**：admin（超级管理员）与 operator（运营）权限区分，三层保护。
...
#### Step 4.3 ✅ 后端 - 数据统计聚合接口（2h）— 2026-04-18 完成
...
#### Step 4.4 ✅ 前端 - 数据统计看板页面（2h）— 2026-04-18 完成
- 依赖: Step 4.3
...
#### Step 4.11 ✅ 管理后台 - 订单列表 + 用户列表导出 Excel（1.5h）— 2026-04-20 完成
...
#### 管理员管理模块 ✅ — 2026-04-21 完成（计划外新增）
- 后端：`AdminManagerService` 管理员 CRUD + `AdminController` 5 个端点（列表/新增/禁用/重置密码/删除）
- 前端：`AdminManagerView.vue` 管理员管理页面 + `adminManager.js` API 模块
- 路由和菜单：`/admins` 路由 + 侧边栏"管理员管理"入口
- 自我保护：不能禁用/删除自己的账号
- **验证**：管理员列表分页搜索、新增、启用/禁用、重置密码、删除均正常

#### 创建回收员改为新建用户 + operator 权限控制 ✅ — 2026-04-22 完成（计划外新增）
- 删除"创建回收员"功能，替换为"新建用户"功能（创建普通用户 USER）
- 新增 operator（运营）与 admin（超级管理员）权限区分
- 三层保护：前端菜单 `v-if="adminStore.isSuperAdmin"` + 路由守卫 `meta.requiresSuperAdmin` + 后端 `checkSuperAdmin()` API 校验
- **验证**：operator 登录后看不到管理员管理菜单，直接访问 /admins 被重定向，调用 API 返回 403

#### 密码 BCrypt 加密 ✅ — 2026-04-22 完成（计划外新增）
- 全部密码存储和校验从明文改为 Hutool BCrypt 加密（`cn.hutool.crypto.digest.BCrypt`）
- 涉及 6 处：用户登录/注册/改密码、管理员登录、管理员创建用户/重置密码、管理员创建管理员/重置管理员密码
- SQL 测试数据全部更新为 BCrypt 密文
- **验证**：所有角色登录、注册、改密码均正常

### Day 28：最终收尾

#### Step 4.12 🔴 体验优化与演示准备（6h）
