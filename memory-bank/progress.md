...
**完成内容：**
- `RecycleOrderMapper.java`：新增 4 个 @Select 统计方法（按日统计、按状态统计、汇总积分重量、回收员排行）
- `AdminStatisticsService.java`：新建统计聚合服务，返回 overview + orderTrend + orderStatusDistribution + categoryDistribution + collectorRanking
- `AdminController.java`：新增 `GET /api/admin/statistics` 端点
...
**完成内容：**
- 安装 ECharts 依赖
- 改造 `DashboardView.vue`：顶部 4 个数据概览卡片 + 2×2 图表网格（订单趋势折线图、状态分布饼图、品类分布柱状图、回收员排行柱状图）
- `api/statistics.js`：统计数据 API 模块
...
**完成内容：**
- 新建 `pages/complaint/list/list.vue`：申诉列表页面（类型、描述、状态、处理结果）
- `pages.json` 添加申诉列表路由
- `pages/user/user.vue` 个人中心新增"我的申诉"入口
...
**完成内容：**
- 新建 `pages/points/history/history.vue`：积分收支明细页面（类型、金额、余额、时间、关联订单）
- `pages.json` 添加积分明细路由
- `pages/user/user.vue` 个人中心新增"积分明细"入口

---

### Step 4.11 🟡 管理后台 - 订单+用户列表导出 Excel（1.5h）— ✅ 2026-04-20 完成
